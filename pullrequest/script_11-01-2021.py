...
class Repository:
                    ...
                    for i in range(1, 1000):
                        try:
                            ...
                            data = requests.get(url,
                                                auth=HTTPBasicAuth(username[g_counter % 5],
                                                                   password[g_counter % 5])).json()
                            if data is not None:
                                if len(data) == 0:
                                    break
                                for k in data:
                                    ...
                                    if Repository.june_2019 >= datetime.strptime(commit_date, '%Y-%m-%dT%XZ'):
                                        l.append([k['commit']['message'], commit_date, k['sha']])
    ...
    def get_all_pull_commits(url):
        global username, password, g_counter
        data = requests.get(url, auth=HTTPBasicAuth(username[g_counter % 5], password[g_counter % 5])).json()
        l = []
        commits_sha = []
        if data is not None:
            if len(data) == 0:
        ...
        # try:
            ...
            for i in range(1, 1000):
                # try:
                    ...
                    global username, password, g_counter
                    data = requests.get(url,
                                        auth=HTTPBasicAuth(username[g_counter % 5], password[g_counter % 5])).json()
                    if data is not None:
                        if len(data) == 0:
                            return l
                        for k in data:
                            _from = k['head']['label']
                            _in_to = k['base']['label']
                            if k['head']['repo'] is None:
                                _from_repo = 'not found repository may be this repo deleted'
                            else:
                                _from_repo = k['head']['repo']['full_name']
                            ...
                            if Repository.june_2019 >= datetime.strptime(pull_date, '%Y-%m-%dT%XZ'):
                                l.append([k['title'], pull_date, _from, list_of_commits, list_commits_sha, k['id'],
                                          k['merge_commit_sha'], _in_to, _from_repo])
    ...
    def write_to_excel_file(
        ...
        sheet_parent_pull_request.write(0, 4, 'list of commit sha that they contain')
        sheet_parent_pull_request.write(0, 5, 'id')
        sheet_parent_pull_request.write(0, 6, 'merged commit sha')
        sheet_parent_pull_request.write(0, 7, 'into')
        sheet_parent_pull_request.write(0, 8, 'from_repo')
        row = 1
        for item in parent_pull_request:
            ...
            sheet_parent_pull_request.write(row, 4, str(item[4]))
            sheet_parent_pull_request.write(row, 5, str(item[5]))
            sheet_parent_pull_request.write(row, 6, str(item[6]))
            sheet_parent_pull_request.write(row, 7, str(item[7]))
            sheet_parent_pull_request.write(row, 8, str(item[8]))
            row += 1
        ...
        sheet_parent_pull_request_author.write(0, 2, 'sha')
        row = 1
        for item in parent_pull_request_author:
            ...
            sheet_parent_pull_request_author.write(row, 2, item[2])
            row += 1
