...
if __name__ == '__main__':
    try:
        ...
        keys = ['full_name', 'repo_id', 'forked_from', 'parent_id', 'hf_parent_id', 'depth', 'hf_depth',
                'is_hard_fork',
                'url', 'counter', 'percent_with_unique_commits_message', 'percent_with_unique_commits_sha',
                'num_of_unique_commits_with_message', 'num_of_unique_commits_with_sha',
                'num_of_commits']
        reposes_inf = []
        for i in range(1, sheet.nrows):
            try:
                ...
                if has_repo(full_name):
                    ...
                    if full_name == 'racker/graylog2-server'or full_name=='madAndroid/graylog2-server':
                        fla = True
                    ...
                    all_repo_commits_message = set()
                    all_parent_commits_message = set()
                    for commit in repo_commit:
                        all_repo_commits_message.add(commit[0])
                    for commit in parent_commit:
                        all_parent_commits_message.add(commit[0])
                    unique_commits_message = all_repo_commits_message.difference(all_parent_commits_message)

                    all_repo_commits_sha = set()
                    all_parent_commits_sha = set()
                    for commit in repo_commit:
                        all_repo_commits_sha.add(commit[3])
                    for commit in parent_commit:
                        all_parent_commits_sha.add(commit[3])
                    unique_commits_sha = all_repo_commits_sha.difference(all_parent_commits_sha)

                    repo_dict['counter'] = counters.get(full_name, 0)
                    if counters.get(full_name, 0) + len(unique_commits_message):
                        repo_dict['percent_with_unique_commits_message'] = '{:.2f}%'.format(
                            counters.get(full_name, 0) / (counters.get(full_name, 0) + len(unique_commits_message)) * 100)
                    else:
                        repo_dict['percent_with_unique_commits_message'] = 'has not unique commit and parent pull request'
                    if counters.get(full_name, 0) + len(unique_commits_sha):
                        repo_dict['percent_with_unique_commits_sha'] = '{:.2f}%'.format(
                            counters.get(full_name, 0) / (counters.get(full_name, 0) + len(unique_commits_sha)) * 100)
                    else:
                        repo_dict['percent_with_unique_commits_sha'] = 'has not unique commit and parent pull request'
                    repo_dict['num_of__unique_commits_with_message'] = len(unique_commits_message)
                    repo_dict['num_of_unique_commits_with_sha'] = len(unique_commits_sha)
