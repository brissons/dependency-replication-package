...
def analyze(repo_commit, parent_commit, parent_pull_request, parent_pull_request_author):
    ...
    for i in range(len_repo_commits):
        ...
        for j in range(len_parent_commit):
            if (repo_commit[len_repo_commits - 1 - i][0] == parent_commit[len_parent_commit - 1 - j][0]) or (
                    repo_commit[len_repo_commits - 1 - i][1] == parent_commit[len_parent_commit - 1 - j][1]) or (
                    repo_commit[len_repo_commits - 1 - i][0] in parent_pull_request) or (
...
if __name__ == '__main__':
    try:
        ...
        workbook = xlsxwriter.Workbook('get_git_qalat.xlsx')
