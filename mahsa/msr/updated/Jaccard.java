...
public class Jaccard {
    ...
    private static void jaccard() throws Exception {
        String file = "NonRepeated_Repos-Forks-recovery.xlsx";
        String path = "C:/Users/mahsa/Desktop/Variability/CleanOfClean/SmallGroup2/";
        ...
        List<String> owner_nameL = Pick_GeneralNext.pick2(path + file, 0, 2, 1);
        ...
        List<String> full_nameL = Pick_GeneralNext.pick2(path + file, 0, 3, 1);
        List<Double> depth = Pick_GeneralNumeric.pick_3(path + file, 0, 4, 1);
        ...
        datas = new Object[]{"Repos_id", "Reos_name", "fullname", "Owner_name", "Created_at", "Depth", "Commits", "TotalDependencies", "Dependencies", "", "CommitsAfter", "DependAfter", "DependAfter_Details", "JaccardDistance", "JaccardSimmilarity","Group"};
        allobj.add(datas);
        ...
        for (int i = 0; i < list_set.size(); i++) {
            ...
            for (int j = 0; j < repo_nameL.size(); j++) {
                if (list_set.get(i).equals(repo_nameL.get(j))) {
                    ...
                    int group = 1;
                    if (jaccardDistance <= 0.50) {
                        group = 0;
                    }
                    datas = new Object[]{Repos_id.get(j), repo_nameL.get(j), full_nameL.get(j), owner_nameL.get(j), Created_at.get(j), depth.get(j),  Commits.get(j), TotalDependencies.get(j), Dependencies.get(j), "", CommitsAfter.get(j), DependAfter.get(j), DependAfter_Details.get(j), jaccardDistance, jaccardSimilarity, group};
                    allobj.add(datas);

                }
            }

            Create_Excel.createExcel2(allobj, 0, path + "ReposFork_Dependencies_version_jaccard_0003.xlsx", "jaccard");
