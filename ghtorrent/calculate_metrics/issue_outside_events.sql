...
(
	...
	from
		(
		select outside_issues.repo_id,  dependencies.final_data.ancestor,  outside_issues.id, ghtorrent_restore.issue_events.action
		from 
	(
		...
		on outside_issues.id = ghtorrent_restore.issue_events.issue_id
		join dependencies.final_data_users 
        on ghtorrent_restore.issue_events.actor_id = dependencies.final_data_users.user_id
        join dependencies.final_data
        on dependencies.final_data.repo_id = outside_issues.repo_id
		where action="subscribed"
		and dependencies.final_data_users.repo_id =  outside_issues.repo_id
	 )as issue_metric
	group by repo_id
